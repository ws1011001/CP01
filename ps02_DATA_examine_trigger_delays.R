...
##
## SCRIPT to examine the timing differences between recorded triggers and E-prime log, and to estimate the temporal
...
for (i in 1:length(subjects)){
  # setup working path and read data
  subj <- subjects[i]
  ...
  # remove short delays (<1ms)
  if (isCleanShortDelays){
    timings <- timings[timings$delays > 1 | timings$delays < -1,]  # only remain timings with delay greater than 1 ms
  }
  # plot delays distribution for each condition
  plist <- lapply(unique(timings$conditions), function(x) 
                  qplot(timings$delays[timings$conditions == x], geom = "histogram", binwidth = 1, xlab = x, ylim = c(0, 10)))
...
## plot distributions of timing delays for all subjects
# by subjects
plist_subject <- lapply(unique(timings_all$subject), function(x) 
                        qplot(timings_all$delays[timings_all$subject == x], geom = "histogram", binwidth = 1, xlab = x))
...
# by conditions
plist_conditions <- lapply(unique(timings_all$conditions), function(x) 
                           qplot(timings_all$delays[timings_all$conditions == x], geom = "histogram", binwidth = 1, xlab = x, ylim = c(0, 30)))
...
# by settings
plist_settings <- lapply(unique(timings_all$settings), function(x) 
                         qplot(timings_all$delays[timings_all$settings == x], geom = "histogram", binwidth = 1, xlab = x))
