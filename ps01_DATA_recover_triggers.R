...
##
## By Shuai Wang, [date] 2021-02-19
##
## ---------------------------
## Notes: - This script shold be performed on certain subjects whose triggers are incomplete.
##   
##
## ---------------------------
...
subj <- 'sub-test'             # input the ID of the subject whose triggers are incomplete
wdir <- file.path(mdir, subj)  # working folder
frec <- sprintf('%s_ses-01_task-RS_run-01_events_recovered.csv', subj)
# setup parameters
blocks <- c(1)      # the blocks that have missing triggers; sub-04: c(2,3,4,5); sub-06: c(1,3,4,5)
...
for (i in 1:length(blocks)){
  ib <- blocks[i]  # block number
  # read recorded triggers
  ...
  # clean up conditions names of edat
  ...
  edat[is.na(edat)] <- 0
  edat$onsets_recorded <- edat$StimAud.OnsetTime + edat$StimVDO.OnsetTime + edat$StimVis.OnsetTime  # group stimuli timings
